...
async function notifyRequestSubmitted({ db, applicant, items, projectLocation }) {
  ...
  await sendToMultipleUsers({
    ...
    data: {
      thing1: { value: applicant },
      thing2: { value: _truncate(itemsSummary, 20) },
      time3: { value: _formatTime(new Date()) },
      thing5: { value: _truncate(projectLocation || '未填写', 20) },
    },
...
async function notifyApprovalResult({ db, applicantUsername, result, items, reason }) {
  ...
  await sendSubscribeMessage({
    ...
    data: {
      phrase1: { value: result === 'approved' ? '已通过' : '已驳回' },
      thing2: { value: _truncate(itemsSummary, 20) },
      time3: { value: _formatTime(new Date()) },
      thing4: { value: _truncate(reason || (result === 'approved' ? '审批通过' : '无'), 20) },
    },
...
async function notifyStockAlert({ db, partNo, partName, currentStock, minStock }) {
  ...
  await sendToMultipleUsers({
    ...
    data: {
      ...
      thing4: { value: `缺口${minStock - currentStock}件，请及时补货` },
    },
