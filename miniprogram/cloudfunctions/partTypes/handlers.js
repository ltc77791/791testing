/**
 * PartTypes handlers — 云函数版本
 * 从 server/src/handlers/partTypes.js 适配
 */
...
async function listPartTypes(req, res) {
  try {
    ...
    const [items, total] = await Promise.all([
      db.collection('part_types').find(filter).sort({ updated_at: -1 }).skip(skip).limit(limit).toArray(),
...
async function createPartType(req, res) {
  try {
    ...
    const doc = {
      part_no, part_name, min_stock: Number(min_stock) || 0,
      current_stock: 0, total_outbound: 0, updated_at: new Date(),
    };

    await db.collection('part_types').insertOne(doc);
    await db.collection('sys_logs').insertOne({
      category: 'PartType', action_type: '新增备件类型', operator: req.user.username,
      details: `新增备件类型: ${part_no} - ${part_name}, 安全库存: ${doc.min_stock}`, created_at: new Date(),
    });
...
async function updatePartType(req, res) {
  try {
    ...
    const changes = [];
    if (part_name !== undefined) { updateFields.part_name = part_name; changes.push(`名称: ${part_name}`); }
    if (min_stock !== undefined) { updateFields.min_stock = Number(min_stock); changes.push(`安全库存: ${min_stock}`); }

    if (changes.length === 0) {
      return res.status(400).json({ code: 1, message: '没有需要更新的字段' });
    }

    await db.collection('part_types').updateOne({ part_no }, { $set: updateFields });

    if (part_name !== undefined) {
      await db.collection('inventory').updateMany({ part_no }, { $set: { part_name } });
    }

    await db.collection('sys_logs').insertOne({
      category: 'PartType', action_type: '编辑备件类型', operator: req.user.username,
      details: `编辑备件类型 ${part_no}: ${changes.join(', ')}`, created_at: new Date(),
    });
...
async function deletePartType(req, res) {
  try {
    ...
    if (invCount > 0) {
      return res.status(400).json({ code: 1, message: `该备件类型下有 ${invCount} 条库存记录，无法删除` });
    }

    const reqCount = await db.collection('requests').countDocuments({ part_no, status: 'pending' });
    if (reqCount > 0) {
      return res.status(400).json({ code: 1, message: `该备件类型有 ${reqCount} 条待审批申请，无法删除` });
    }

    await db.collection('part_types').deleteOne({ part_no });
    await db.collection('sys_logs').insertOne({
      category: 'PartType', action_type: '删除备件类型', operator: req.user.username,
      details: `删除备件类型: ${part_no} - ${existing.part_name}`, created_at: new Date(),
    });
