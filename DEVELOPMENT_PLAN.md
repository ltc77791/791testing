...
### 后续阶段：PC-微信互通 + 回归测试 + 发布

| 步骤 | 任务 | 说明 | 状态 |
|:----:|------|------|:----:|
| 7-1 | PC 管理后台接入云数据库 | PC 后台改用微信云数据库，打通双端数据 | ⬜ 待执行 |
| 7-2 | 回归测试 | 全流程端到端回归，确保无遗漏 | ⬜ 待执行 |
| 7-3 | 体验版内测 | 上传代码 → 体验版 → 团队内测 3-5 天 | ⬜ 待执行 |
| 7-4 | 提交审核 + 正式发布 | 微信公众平台提交审核 → 通过后全量发布 | ⬜ 待执行 |
| 7-5 | PC 管理后台部署 | `npm run build` → 静态托管上传 | ⬜ 待执行 |
...
```
 ✅ 已完成                                                          ← 当前位置 →            后续
──────────────────────────────────────────────────────────────────┬──────────────────┬──────────────────
6-2-1~4  部署+数据+索引+登录联调                                    │                  │ 7-2 回归测试
6.5      API 验证 (8项通过)                                        │                  │     ↓
6-3-1~4  核心业务联调 ✅                                            │ 7-1 PC-微信互通   │ 7-3 体验版内测
6-4-1~4  申请审批联调 ✅                                            │   (数据库打通)     │ 7-4 提交审核+发布
6-5-1~5  订阅消息通知 ✅                                            │                  │ 7-5 PC后台部署
6-5-UI   按钮样式修复 ✅                                            │                  │
──────────────────────────────────────────────────────────────────┴──────────────────┴──────────────────
...
> 当前状态: PC 管理后台使用本地 Express + MongoDB，微信小程序使用微信云函数 + 云数据库。两端数据库独立，无法实时同步。以下为几种互通方案的对比与建议。

### 方案对比

| # | 方案 | 原理 | 优点 | 缺点 | 推荐度 |
|:-:|------|------|------|------|:------:|
| A | **PC 后台直连微信云数据库** | 使用微信云开发 HTTP API（`tcb-manager-node` / 云开发 HTTP 接口），PC 后端直接读写云数据库 | 零数据延迟，架构最简，无需维护同步 | 需获取云环境访问密钥（SecretId/SecretKey），受云开发 QPS 限制，部分复杂聚合能力受限 | ⭐⭐⭐⭐⭐ |
| B | **统一迁移至云托管容器** | 将 Express 后端部署到微信云托管（Docker 容器），共享云数据库 | 天然互通，Express 代码几乎不改，支持自定义域名 | 需 Docker 化部署，有容器资源费用，调试不如本地方便 | ⭐⭐⭐⭐ |
| C | **中间件双向同步** | 部署同步服务（如 MongoDB Change Stream → 云数据库），实时双向同步数据 | 两端可独立运行，保留现有架构 | 架构最复杂，数据冲突处理困难，延迟不可控 | ⭐⭐ |
| D | **云函数作为统一 API 网关** | PC 后台也通过 HTTP 调用云函数，云函数作为唯一 API 层 | 数据一致性最好，API 逻辑统一 | PC 端每次请求都走云函数，延迟较高，本地开发体验差 | ⭐⭐⭐ |

### 推荐方案：A — PC 后台直连微信云数据库

**选择理由**:
1. 改动最小 — 只需替换 `server/src/db.js` 的数据库连接层，handlers 业务逻辑基本不变
2. 已有基础 — 项目已有 `db-adapter.js` 双模适配层的设计思路，可复用
3. 零数据延迟 — PC 后台和小程序操作同一数据库，无同步问题
4. 成本最低 — 无需额外服务器或容器资源
...
| 步骤 | 任务 | 说明 |
|:----:|------|------|
| 7-1-1 | 获取云环境访问凭证 | 微信公众平台 → 云开发 → 设置 → 获取云环境 ID + 腾讯云 API 密钥 (SecretId/SecretKey) |
| 7-1-2 | 安装 `@cloudbase/node-sdk` | PC 后端安装腾讯云开发 Node.js SDK，配置环境变量 |
| 7-1-3 | 改造 `server/src/db.js` | 新增云数据库连接模式，通过 `DB_MODE` 环境变量切换本地 MongoDB / 云数据库 |
| 7-1-4 | 适配数据库操作 API | 云数据库 API 与 MongoDB 原生驱动有差异（如 `_id` 格式、聚合语法），需逐个适配 |
| 7-1-5 | PC 后台认证调整 | 生产环境 PC 后台保留 JWT 登录（非微信扫码），但读写同一云数据库 |
| 7-1-6 | 联调验证 | PC 端操作（入库/编辑/审批）→ 小程序端实时可见，反之亦然 |

**关键配置** (`.env` 新增):
```
DB_MODE=cloud                          # local | cloud
TCB_ENV_ID=your-cloud-env-id           # 微信云开发环境 ID
TCB_SECRET_ID=your-secret-id           # 腾讯云 API SecretId
TCB_SECRET_KEY=your-secret-key         # 腾讯云 API SecretKey
```

### 备选方案：B — 云托管容器部署

如果方案 A 遇到云数据库聚合能力不足（如复杂的 `$lookup`、`$bucket` 等分析查询），可升级为方案 B:

1. 将 Express 后端打包为 Docker 镜像
2. 部署到微信云托管（CloudBase Run）
3. 云托管容器内可直接连接云数据库的内网地址，性能更好
4. PC 前端通过自定义域名访问云托管容器中的 Express API
5. 小程序端仍走云函数（或也改为调用云托管 API）

**注意**: 方案 B 的月成本约 ¥30-100（取决于容器规格和调用量），适合正式上线后的长期运行。
