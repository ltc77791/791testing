...
> 系统编码与云函数部署已全部完成，以下为上线前的收尾与运维工作

### Phase A：数据与索引准备（上线前必做）

| # | 任务 | 操作方式 | 优先级 |
|:-:|------|---------|:------:|
| A1 | 云数据库创建索引 | 云开发控制台手动创建，参照 `scripts/create-cloud-indexes.js` 清单（10 个索引） | 🔴 高 |
| A2 | 生产数据导入 | 用 `scripts/mongo2wx.js` 将现有数据转为 JSONL，通过云控制台批量导入 | 🔴 高 |
| A3 | 创建生产环境管理员账号 | 云数据库 users 集合手动插入 admin 用户（bcrypt 哈希密码） | 🔴 高 |

### Phase B：PC 管理后台部署

| # | 任务 | 操作方式 | 优先级 |
|:-:|------|---------|:------:|
| B1 | 前端构建 | `cd admin && npm run build`，产出 `dist/` 目录 | 🟡 中 |
| B2 | 静态托管上传 | 云开发控制台 → 静态网站托管 → 上传 `dist/` 全部文件 | 🟡 中 |
| B3 | 自定义域名（可选） | 静态托管绑定自有域名 + HTTPS 证书 | 🟢 低 |

### Phase C：小程序发布

| # | 任务 | 操作方式 | 优先级 |
|:-:|------|---------|:------:|
| C1 | 体验版测试 | 微信开发者工具 → 上传代码 → 设为体验版 → 邀请团队内测 | 🟡 中 |
| C2 | 审核提交 | 微信公众平台 → 版本管理 → 提交审核 | 🟡 中 |
| C3 | 正式发布 | 审核通过后全量发布 | 🟡 中 |
...
| # | 任务 | 说明 | 优先级 |
|:-:|------|------|:------:|
| D1 | 云函数日志监控 | 云开发控制台 → 云函数 → 日志，关注错误率和耗时 | 🟡 中 |
| D2 | 数据库备份策略 | 开启云数据库自动备份，设置每日备份 | 🟡 中 |
| D3 | 性能调优 | 根据实际使用量调整云函数内存/超时配置 | 🟢 低 |
| D4 | 用户反馈迭代 | 收集一线使用反馈，持续改进 UI/UX | 🟢 低 |

### 建议执行顺序

```
A1 → A2 → A3 → C1（体验版内测）→ B1 → B2 → C2 → C3 → D1 → D2
 索引    数据    账号    小程序内测    PC构建  PC部署  小程序审核  发布   监控   备份
```

**关键提醒**:
- 索引必须在数据导入前创建（唯一索引要求集合无重复数据）
- 体验版内测建议至少 3-5 天，覆盖完整业务流程
- PC 管理后台的 API 请求需从云函数走，确认前端 `http.ts` 的 baseURL 适配云环境
