...
─────────────────────────────────────────────────────────────────
下一阶段  云托管部署 + 数据库互通 (阶段 7, 方案 B-1)  ⬜ 待执行
          回归测试 + 发布 (阶段 8)                    ⬜ 待执行
```
...
### 阶段 7：云托管部署 + PC-微信互通 (方案 B-1)

> 目标: Express Docker 化部署到微信云托管，连接腾讯云 MongoDB，实现 PC 端和小程序端共享同一数据库
> 前置条件: 见第九章 § 9.4 开发前检查清单

| 步骤 | 任务 | 说明 | 状态 |
|:----:|------|------|:----:|
| 7-0 | 环境准备 | 腾讯云账号 + 域名购买 + ICP 备案 + MongoDB 实例 + 云托管开通 | ⬜ 待执行 |
| 7-1 | Express Docker 化 | 编写 Dockerfile + container.config.json，本地验证镜像 | ⬜ 待执行 |
| 7-2 | db.js 改读环境变量 | `MONGO_URI` 环境变量，本地/云端自动切换 | ⬜ 待执行 |
| 7-3 | 云托管部署 + MongoDB 联通 | 推镜像到云托管，配置 VPC 内网连接 MongoDB | ⬜ 待执行 |
| 7-4 | 数据迁移 | 云开发数据库 → 腾讯云 MongoDB (JSON 导出导入) | ⬜ 待执行 |
| 7-5 | 云函数改连 MongoDB | 云函数从云开发数据库切换到腾讯云 MongoDB | ⬜ 待执行 |
| 7-6 | PC 前端部署 | `npm run build` → 静态托管/CDN + 自定义域名 + SSL | ⬜ 待执行 |
| 7-7 | 双端联调验证 | PC 操作 ↔ 小程序端实时可见，全流程回归 | ⬜ 待执行 |

### 阶段 8：回归测试 + 发布

| 步骤 | 任务 | 说明 | 状态 |
|:----:|------|------|:----:|
| 8-1 | 全流程回归测试 | PC 端 + 小程序端完整业务流程验证 | ⬜ 待执行 |
...
```
 ✅ 已完成                                            ← 当前位置 →                     后续
────────────────────────────────────────────────┬──────────────────────────────┬─────────────────
6-2-1~4  部署+数据+索引+登录联调                  │                              │
6.5      API 验证 (8项通过)                      │ 7-0 环境准备                   │ 8-1 全流程回归
6-3-1~4  核心业务联调 ✅                          │   ├ 腾讯云账号+实名            │     ↓
6-4-1~4  申请审批联调 ✅                          │   ├ 域名购买+ICP备案 (7-25天)  │ 8-2 体验版内测
6-5-1~5  订阅消息通知 ✅                          │   ├ MongoDB 实例购买           │     ↓
6-5-UI   按钮样式修复 ✅                          │   └ 云托管开通                 │ 8-3 提交审核+发布
                                                │ 7-1~3 Docker化+部署+联通       │
                                                │ 7-4~5 数据迁移+云函数改连       │
                                                │ 7-6~7 前端部署+双端联调         │
────────────────────────────────────────────────┴──────────────────────────────┴─────────────────

⚠️ 上线前阻塞项: ICP 域名备案 (7-25 天)，建议立即启动
```
...
### 9.2 最终决策：方案 B-1 (云托管 + 腾讯云 MongoDB)

**选择理由**:
1. **Express 代码零改动** — 全部 8 个 handler 文件不需要改写，只需 `db.js` 改读环境变量
2. **MongoDB 全功能** — `$lookup`/`$dateDiff`/`$facet`/`$merge` 等全部可用，未来分析扩展无限制
3. **内网低延迟** — 云托管容器与腾讯云 MongoDB 在同 VPC 内网连接，延迟 <1ms
4. **统一数据源** — PC 后台 (云托管 Express) 和小程序 (云函数) 通过 SDK/HTTP 共享同一 MongoDB
...
当前小程序使用的微信云开发数据库中的数据，需要迁移到腾讯云 MongoDB:
1. 微信云开发控制台 → 数据库 → 导出为 JSON
2. 使用已有的 `scripts/mongo2wx.js` 反向转换 (或 `mongoimport` 直接导入)
3. 云函数改为通过 `@cloudbase/node-sdk` 连接腾讯云 MongoDB (替代云开发数据库)
4. 验证小程序端功能正常
...
| 云函数: `_shared/` | 需改造 | 云函数需改为连接腾讯云 MongoDB 而非云开发数据库 |
...
生产部署:
  npm run build (admin)  → dist/ → 上传静态托管 (CDN)
  docker build (server)  → 镜像 → 推到微信云托管
  云函数                  → 改连腾讯云 MongoDB 内网地址
```
...
| 6 | 已备案域名 | PC 浏览器访问必须备案 | ¥50-70/年 (域名) + 备案免费 | ⬜ 需购买+备案 |
...
| # | 项目 | 说明 | 费用 |
|:-:|------|------|------|
| 9 | CDN 加速 | PC 前端静态资源加速 | 按流量计费，小流量几乎免费 |
| 10 | 云监控告警 | 容器/数据库异常通知 | 免费 |
| 11 | 日志服务 (CLS) | 容器日志持久化 | 免费额度内够用 |

#### 月度成本估算

| 项目 | 最低配置 | 月费 |
|------|---------|-----:|
| 微信云开发 (小程序云函数) | 免费/基础版 | ¥0-19.9 |
| 微信云托管 (Express 容器) | 0.25核 512MB × 1 实例 | ¥20-50 |
| 腾讯云 MongoDB (副本集) | 2GB 内存 × 3 节点 + 50GB 存储 | ¥200-545 |
| 域名 | .com / .cn | ¥5/月 (年付) |
| **合计** | | **¥225-620/月** |

> MongoDB 是最大开销。可关注腾讯云新用户优惠/活动折扣降低成本。
...
小程序内部调用不需要备案 (走微信私有协议)，但 PC 浏览器访问必须备案。
...
□ 4. 开通微信云托管 (微信云托管控制台)
□ 5. 购买腾讯云 MongoDB 实例 (与云托管同地域同 VPC)
□ 6. 确认 MongoDB 实例与云托管网络互通 (VPC 设置)
□ 7. 申请免费 SSL 证书
□ 8. 安装 Docker Desktop (本地开发调试)
```

其中 1-3 可立即启动，不影响编码工作。4-6 在编码阶段随时可以做。
