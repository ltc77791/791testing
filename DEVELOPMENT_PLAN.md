...
| 步骤 | 任务 | 验证要点 | 状态 |
|:----:|------|---------|:----:|
| 6-3-1 | 首页 KPI + 安全库存预警 | 绑定用户登录后看到正确统计数据，预警列表与实际库存一致 | ⬜ 待验证 |
| 6-3-2 | 扫码查询 + 扫码入库 | 扫码获取 SN → 显示备件详情；入库模式填表提交 → 库存+1 | ⬜ 待验证 |
| 6-3-3 | 库存列表 + 搜索筛选 | 分页加载、下拉刷新、状态筛选、关键词搜索均正常 | ⬜ 待验证 |
| 6-3-4 | 备件类型管理 | 类型列表正常加载（供申请出库下拉使用） | ⬜ 待验证 |

### 下一阶段：申请审批联调（依赖核心业务）

| 步骤 | 任务 | 验证要点 | 状态 |
|:----:|------|---------|:----:|
| 6-4-1 | 提交申请 | 选备件类型+数量 → 提交 → 库存预留 → 申请列表可见 | ⬜ 待验证 |
| 6-4-2 | 我的申请列表 + 撤回 | 状态筛选、详情查看、撤回操作（释放预留） | ⬜ 待验证 |
| 6-4-3 | 审批通过（含部分批准） | manager 审批 → 库存出库 → 状态流转 → 日志记录 | ⬜ 待验证 |
| 6-4-4 | 驳回 | 驳回+原因 → 释放预留 → 申请人看到驳回状态 | ⬜ 待验证 |

### 后续阶段：新功能 + 发布

| 步骤 | 任务 | 说明 | 状态 |
|:----:|------|------|:----:|
| 6-5-1 | 订阅消息通知（申请提交） | 申请人提交后，manager 收到微信通知 | ⬜ 待开发 |
| 6-5-2 | 订阅消息通知（审批结果） | 审批/驳回后，申请人收到微信通知 | ⬜ 待开发 |
| 6-5-3 | 订阅消息通知（安全库存预警） | 库存低于阈值时通知 manager | ⬜ 待开发 |
| 6-5-5 | 回归测试 | 全流程端到端回归，确保无遗漏 | ⬜ 待执行 |
...
```
 ✅ 已完成                          ← 当前位置 →                     后续
─────────────────────────────────┬──────────────────────────────┬──────────────────
6-2-1 部署云函数                  │ 6-3-1~4 核心业务联调 (可并行)  │ 6-5-1~3 消息通知
6-2-2 导入数据                    │         ↓                    │     ↓
6-2-3 建索引                      │ 6-4-1~4 申请审批联调          │ 6-5-5 回归测试
6-2-4 登录联调                    │                              │     ↓
6.5   API 验证 (8项通过)          │                              │ 6-6 体验版内测
                                 │                              │ 6-7 提交审核+发布
                                 │                              │ 6-8 PC后台部署
─────────────────────────────────┴──────────────────────────────┴──────────────────
```
