 ...
 * KPI 卡片: 在库总数、累计出库、待审批、本月入库、本月出库 + 环比
 */
async function getKPI(req, res) {
  try {
    ...
    // Read from counters (fast path)
    const stats = await db.collection('counters').findOne({ _id: 'stats' });

    // Compute real-time values from inventory for accuracy
    ...
    const [inStock, outStock, pendingCount, monthIn, monthOut, lastMonthIn, lastMonthOut] =
      await Promise.all([
        ...
        db.collection('inventory').countDocuments({
          inbound_time: { $gte: lastMonthStart, $lte: lastMonthEnd },
        }),
        db.collection('inventory').countDocuments({
          status: 1,
          outbound_time: { $gte: lastMonthStart, $lte: lastMonthEnd },
        }),
      ]);
    ...
    res.json({
      ...
      data: {
        ...
        last_month_inbound: lastMonthIn,
        last_month_outbound: lastMonthOut,
 ...
 * 安全库存预警: current_stock < min_stock 的备件类型
 */
  ...
  try {
    ...
    const alerts = await db.collection('part_types').aggregate([
      ...
      },
      {
        $addFields: {
          actual_stock: {
            $ifNull: [{ $arrayElemAt: ['$stock_info.count', 0] }, 0],
          },
        },
      },
  ...
  try {
    ...
    const [inboundTrend, outboundTrend] = await Promise.all([
      ...
      db.collection('inventory').aggregate([
        ...
        {
          $group: {
            _id: { $dateToString: { format: '%Y-%m', date: '$inbound_time' } },
            count: { $sum: 1 },
          },
        },
        { $sort: { _id: 1 } },
      ...
      // 出库按月 (以 outbound_time 为准)
      db.collection('inventory').aggregate([
        { $match: { status: 1, outbound_time: { $ne: null } } },
        {
          $group: {
            _id: { $dateToString: { format: '%Y-%m', date: '$outbound_time' } },
            count: { $sum: 1 },
          },
        },
        { $sort: { _id: 1 } },
    ...
    const monthMap = {};
    for (const r of inboundTrend) {
      monthMap[r.month] = { month: r.month, inbound: r.inbound, outbound: 0 };
    }
    for (const r of outboundTrend) {
      if (monthMap[r.month]) {
        monthMap[r.month].outbound = r.outbound;
      } else {
        monthMap[r.month] = { month: r.month, inbound: 0, outbound: r.outbound };
      }
    }
...
/**
 * GET /api/analytics/consumption?months=3
 * 备件消耗排行 Top 10 + 项目用量统计
 */
async function getConsumption(req, res) {
  try {
    ...
    // Our requests store items as an array; unwind to aggregate per part_no
...
async function getAge(req, res) {
  try {
    ...
    const staleItems = await db.collection('inventory').aggregate([
      ...
      {
        $addFields: {
          age_days: { $dateDiff: { startDate: '$inbound_time', endDate: new Date(), unit: 'day' } },
        },
      },
      { $sort: { age_days: -1 } },
      { $limit: 100 },
