 ...
 * GET /api/logs
 * 系统日志分页查询
 * Query params: category, operator, start_date, end_date, page, page_size
 */
async function getLogs(req, res) {
  try {
    ...
    if (category) {
      // Support comma-separated: "Inbound,Outbound"
    ...
    if (operator) {
      ...
      if (ops.length === 1) {
        filter.operator = ops[0];
      } else if (ops.length > 1) {
        filter.operator = { $in: ops };
      }
    }
    ...
    if (start_date || end_date) {
      ...
      if (end_date) {
        // end_date inclusive: set to end of day
